// Concordance model 7.2: Use a high-quality proxy for SES in place of SES (i.e, without
// trying to impute)
functions {
...
model {
  ...
  I0 ~ normal(SES_mean, sigus);	// Include SES truth value.
  
  // medicaid: base rate + SES effect.
  for (i in 1:N) {
    pD[i] = ad - bud*I0[i]; // Effect of higher SES presumed to be negative.
  }
  D ~ bernoulli_logit(pD);

  // outcome
  for (i in 1:N) {
    pO[i] = ao + bso*S[i] + bho*H[i] + buo*I0[i];
  }

// Concordance model 8.4: Impute SES using the low-quality proxy and
// medicaid status.  Ignore connection between SES and ADM
functions {
...
parameters {
  ...
  vector[n_adm-1] badmo;	// ADM effects on O.
}
...
model {
  ...
  // outcome.  Note the coefficient between adm[i] == n_adm and O is fixed at zero
  for (i in 1:N) {
